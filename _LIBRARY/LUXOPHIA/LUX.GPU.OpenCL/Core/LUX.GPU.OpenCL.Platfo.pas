...
begin
     CheckCL( clGetPlatformInfo( Handle, Name_, SizeOf( _TYPE_ ), @Result, nil ), 'TCLPlatfo.GetInfo is Error!' );
end;

function TCLPlatfo<TCLSystem_>.GetInfoSize( const Name_:T_cl_platform_info ) :T_size_t;
begin
     CheckCL( clGetPlatformInfo( Handle, Name_, 0, nil, @Result ), 'TCLPlatfo.GetInfoSize is Error!' );
end;
...
begin
     ...
     CheckCL( clGetPlatformInfo( Handle, Name_, S, @Result[ 0 ], nil ), 'TCLPlatfo.GetInfos is Error!' );
end;
...
begin
     ...
     CheckCL( clGetPlatformIDs( PsN, @Ps[0], nil ), 'TCLPlatfos.FindPlatfos is Error!' );
